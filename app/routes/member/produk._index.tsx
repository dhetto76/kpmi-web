...
export const meta: Route.MetaFunction = () => [{ title: "Produk & Jasa | KPMI" }];
...
export async function loader({ request }: Route.LoaderArgs) {
  ...
  return data(
    { products: products ?? [], businessCount: businessCount ?? 0 },
    { headers },
  );
}

export default function ProductListPage({ loaderData }: Route.ComponentProps) {
  const { products: list, businessCount } = loaderData;
