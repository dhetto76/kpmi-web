...
export const meta: Route.MetaFunction = () => [{ title: "Usaha Saya | KPMI" }];
...
export async function loader({ request }: Route.LoaderArgs) {
  ...
  return data({ businesses: businesses ?? [] }, { headers });
}

export default function BusinessListPage({ loaderData }: Route.ComponentProps) {
  const list = loaderData.businesses;
    ...
    <div className="space-y-6">
      ...
      {list.length === 0 ? (
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          {list.map((b) => (
            <Link key={b.id} to={`/dashboard/bisnis/${b.id}`} className="card-lift block">
