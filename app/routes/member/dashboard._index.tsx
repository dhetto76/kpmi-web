...
export const meta: Route.MetaFunction = () => [{ title: "Ringkasan | KPMI" }];
  ...
  return data(
    {
      ...
      profileIncomplete: !profile?.korwil || !profile?.phone,
...
export default function DashboardPage({ loaderData }: Route.ComponentProps) {
  const { fullName, status, profileIncomplete, businesses, productCount } =
    loaderData;
    ...
    <div className="space-y-6">
      <div>
        <h1 className="font-display text-2xl font-extrabold text-maroon-900">
          Assalamu&apos;alaikum, {fullName || "Anggota"}
        </h1>
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        {[
          { icon: User, label: "Status Anggota", value: status ?? "-", isStatus: true },
          { icon: Building2, label: "Usaha Terdaftar", value: String(businesses.length) },
          { icon: Package, label: "Produk & Jasa", value: String(productCount) },
        ].map((s) => (
