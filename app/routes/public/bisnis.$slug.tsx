...
import type { Route } from "./+types/bisnis.$slug";
...
/**
 * Next.js fetched the row twice: once in `generateMetadata` and once in the
 * page. Here `meta` reads what the loader already returned, so the detail page
 * costs one query instead of two.
 */
export const meta: Route.MetaFunction = ({ loaderData }) => {
  if (!loaderData) return [{ title: "Usaha tidak ditemukan | KPMI" }];
  return [
    { title: `${loaderData.business.name} | KPMI` },
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  // `notFound()` in Next.js; a thrown 404 here, caught by the ErrorBoundary.
  if (!business) {
  ...
  return data({ business, products: products ?? [] }, { headers });
}

export default function BusinessDetailPage({ loaderData }: Route.ComponentProps) {
  const { business, products } = loaderData;
