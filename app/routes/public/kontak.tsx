...
export const meta: Route.MetaFunction = () => [{ title: "Kontak | KPMI" }];

export async function loader({ request }: Route.LoaderArgs) {
  const { supabase, headers } = createClient(request);
  // Editable from admin → Pengaturan Sistem.
  const { org_address, org_email, org_phone } = await getSiteSettings(supabase);

  return data({ org_address, org_email, org_phone }, { headers });
}

export default function ContactPage({ loaderData }: Route.ComponentProps) {
  const { org_address, org_email, org_phone } = loaderData;
