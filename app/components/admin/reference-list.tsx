 ...
 * exist but not edit the list that defines their own scope. The action refuses
 * them regardless — this only avoids showing controls that cannot work.
 *
 * One fetcher drives every mutation. These are list-wide operations rendered
 * in a single table, and only one runs at a time, so a shared pending state is
 * what the Next.js `useTransition` gave here too.
 */
export function ReferenceList({
  ...
  const message = fetcher.data?.message ?? null;

  // Clear the transient row state once a mutation has come back cleanly.
  useEffect(() => {
  ...
  function add() {
    ...
    submit({ intent: "create", name });
  }

  function saveRename(id: string) {
    submit({ intent: "rename", id, name: editValue.trim() });
  }
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="relative w-full sm:max-w-md">
          ...
          />
          {/*
            Filters the rows already in hand — the whole list is loaded, so
            this stays client-side rather than becoming a query parameter.
          */}
          <input
            type="search"
            value={query}
                      ...
                      <div className="flex items-center justify-end gap-1.5">
                        ...
                        {confirmingId === value.id ? (
                          <div className="flex items-center gap-1.5">
                            <button
                              type="button"
                              onClick={() => submit({ intent: "delete", id: value.id })}
                              disabled={pending}
